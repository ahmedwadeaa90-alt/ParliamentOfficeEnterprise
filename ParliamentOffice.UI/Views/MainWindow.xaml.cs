...
using ParliamentOffice.UI.Services.Attachments;
using ParliamentOffice.UI.Services.Reports;
using ParliamentOffice.UI.Services.Security;
...
FROM Achievements a LEFT JOIN Organizations org ON org.Id=a.OrganizationId ORDER BY a.ActivityDate DESC, a.Id DESC").DefaultView;
    ...
    {
        if (_permissionService.IsAdmin(AppSession.Role)) return true;
        MessageBox.Show("هذه العملية متاحة للمدير فقط");
        return false;
    }

    private static object ToDbValue(int? value) => value == null ? DBNull.Value : value.Value;
    ...
    private void UpdateOutgoing_Click(object sender, RoutedEventArgs e)
    {
        ...
        Db.Execute(@"UPDATE OutgoingLetters SET OutgoingNumber=$num, OutgoingDate=$date, TargetOrganizationId=$org, CitizenId=$citizen, Subject=$subject, RelatedIncomingId=$related, Priority=$pri, Status=$status, FollowUpRequired=$follow, UpdatedAt=CURRENT_TIMESTAMP WHERE Id=$id",
            new SqliteParameter("$num", OutgoingNumberBox.Text.Trim()), new SqliteParameter("$date", (OutgoingDatePicker.SelectedDate ?? DateTime.Today).ToString("yyyy-MM-dd")), new SqliteParameter("$org", ToDbValue(orgId)), new SqliteParameter("$citizen", ToDbValue(citizenId)), new SqliteParameter("$subject", OutgoingSubjectBox.Text.Trim()), new SqliteParameter("$related", ToDbValue(relatedIncomingId)), new SqliteParameter("$pri", ComboText(OutgoingPriorityBox)), new SqliteParameter("$status", ComboText(OutgoingStatusBox)), new SqliteParameter("$follow", OutgoingFollowBox.IsChecked == true ? 1 : 0), new SqliteParameter("$id", id.Value));
    ...
    private void UpdateIncoming_Click(object sender, RoutedEventArgs e)
    {
        ...
        Db.Execute(@"UPDATE IncomingLetters SET IncomingNumber=$num, IncomingDate=$date, SourceOrganizationId=$org, CitizenId=$citizen, Subject=$subject, Priority=$pri, Status=$status, FollowUpRequired=$follow, UpdatedAt=CURRENT_TIMESTAMP WHERE Id=$id",
            new SqliteParameter("$num", IncomingNumberBox.Text.Trim()), new SqliteParameter("$date", (IncomingDatePicker.SelectedDate ?? DateTime.Today).ToString("yyyy-MM-dd")), new SqliteParameter("$org", ToDbValue(orgId)), new SqliteParameter("$citizen", ToDbValue(citizenId)), new SqliteParameter("$subject", IncomingSubjectBox.Text.Trim()), new SqliteParameter("$pri", ComboText(IncomingPriorityBox)), new SqliteParameter("$status", ComboText(IncomingStatusBox)), new SqliteParameter("$follow", IncomingFollowBox.IsChecked == true ? 1 : 0), new SqliteParameter("$id", id.Value));
    ...
    private void AttachFile(string entityType, int id)
    {
        try
        {
            ...
            _attachmentService.AttachFile(entityType, id, dlg.FileName, AppSession.UserId);
            RefreshCitizenDossierAfterAttachment(entityType, id);
            LoadCitizens();
            MessageBox.Show("تم إرفاق الملف");
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "خطأ إرفاق الملف");
        }
    }

    private void RefreshCitizenDossierAfterAttachment(string entityType, int id)
    {
        var citizenId = _attachmentService.GetLinkedCitizenId(entityType, id);
        if (citizenId != null) LoadCitizenDossier(citizenId.Value);
    }
    ...
    private void OpenFolder(string entityType, int id)
    {
        try
        {
            var folder = _attachmentService.GetFolder(entityType, id);
            Directory.CreateDirectory(folder);
            Process.Start(new ProcessStartInfo(folder) { UseShellExecute = true });
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "خطأ فتح المرفقات");
        }
    }
    private void AttachSelectedFile(DataGrid grid, string entityType)
    {
        var id = SelectedId(grid);
        if (id != null) AttachFile(entityType, id.Value);
    }

    private void OpenSelectedFolder(DataGrid grid, string entityType)
    {
        var id = SelectedId(grid);
        if (id != null) OpenFolder(entityType, id.Value);
    }

    private void AttachOutgoing_Click(object sender, RoutedEventArgs e) => AttachSelectedFile(OutgoingGrid, "Outgoing");
    private void AttachIncoming_Click(object sender, RoutedEventArgs e) => AttachSelectedFile(IncomingGrid, "Incoming");
    private void AttachCitizen_Click(object sender, RoutedEventArgs e) => AttachSelectedFile(CitizensGrid, "Citizen");
    private void OpenOutgoingAttachments_Click(object sender, RoutedEventArgs e) => OpenSelectedFolder(OutgoingGrid, "Outgoing");
    private void OpenIncomingAttachments_Click(object sender, RoutedEventArgs e) => OpenSelectedFolder(IncomingGrid, "Incoming");
    private void OpenCitizenAttachments_Click(object sender, RoutedEventArgs e) => OpenSelectedFolder(CitizensGrid, "Citizen");
...
VALUES($code,$date,$title,$cat,$gov,$district,$org,$desc,$keys,'','',$user)",
            ...
            new SqliteParameter("$org", ToDbValue(orgId)),
...
FROM Achievements a LEFT JOIN Organizations org ON org.Id=a.OrganizationId WHERE a.ActivityDate BETWEEN $from AND $to ORDER BY a.ActivityDate ASC, a.Id ASC", new SqliteParameter("$from", from), new SqliteParameter("$to", to));
    ...
    private void RunReport(string kind, string title)
    {
        try
        {
            ReportResultText.Text = "تم إنشاء التقرير: " + _reportService.RunReport(kind, title, AppSession.UserId);
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "خطأ التقرير");
        }
    }
